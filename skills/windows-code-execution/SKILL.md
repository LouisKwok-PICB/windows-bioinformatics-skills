...
- In one-line QC commands, never write `foreach (...) { if (...) { ... } else { ... } } | Format-Table` or the same pattern before `Export-Csv`. In Windows PowerShell this can fail at parse time with `An empty pipe element is not allowed`. Safe pattern: initialize `$rows = @()`, append `[pscustomobject]` rows inside the loop, then run `$rows | Format-Table` or `$rows | Export-Csv`.
- Do not put a `foreach (...) { ... }` language statement directly inside an array subexpression, command argument, or nested expression such as `$unique = @((foreach ($path in $paths) { ... }) | Sort-Object -Unique)`. Windows PowerShell can parse this as an invalid expression. Use `$rows = foreach ($path in $paths) { ... }`, then `$unique = @($rows.Property | Sort-Object -Unique)`.
